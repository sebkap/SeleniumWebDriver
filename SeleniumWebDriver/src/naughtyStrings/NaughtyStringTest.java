...
public class NaughtyStringTest {
	...
	public void naughtyString() throws Exception {
		...
		// Tworzy obiekt URL, adres zawiera wszytkie naugthy strings w formacie
		// tekstowym
		// (za każdym razem w przypadku aktualizacji jest możliwe pobranie nowej
		// listy)
		URL naughtyStringsTxtUrl = new URL(
				"https://raw.githubusercontent.com/minimaxir/big-list-of-naughty-strings/master/blns.txt");

		// Poniższe komendy z pętlą umożliwiają sparsowanie otrzymanego pliku
		// tekstowego
		// pętla umożliwia zapisanie wyniku w obiekcie List z pominięciem linii
		// zawierających znak #
		...
		// Nasza pętla testowa, która wprowadzi każdy ciąg znaków z listy, a
		// następnie zweryfikuje, czy w danym
		...
		for (int i = 0; i < lines.size(); i++) {
			...
			// Oczekiwanie do 5 na wyświetlenie posta w temacie, gdy go nie
			// będzie zostanie zalogowany jaki String
	...
	// Metoda, która czeka przez określony, aż dany element będzie wyświetlony w
	// danym elemencie znalezionym za pomocą
	// lokatora By
	private boolean explicitWaitElementPresent(WebDriver driver, By byLocator, String textToShowUp, int duration) {
		...
		// Gdy znajdzie weblement i znajduje się się w nim określony tekst
		// zmienna boolean zostanie nadpisana i zwróci wartość true
		// + obsługa wyjątku Timeout, gdy tekst w elemencie np. nie będzie
		// wyświetlony
		try {
